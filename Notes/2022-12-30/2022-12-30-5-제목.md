...
기존 서비스 코드를 보면 DAO 클래스의 객체인 dao에 대해 dao.select()와 dao.delete()를 호출하지만, dao의 각 메서드마다 새로운 Connection을 생성하여 진행하기 때문에 select와 delete 두 작업을 하나의 트랜잭션으로 진행할 수가 없다는 한계가 있었다.
