...
### 일급객체(First-class object, First-class citizen)
어떤 객체가 일급객체가 되는 조건

1. 익명의 literal로 생성 가능해야 함
2. 변수에 저장될 수 있어야 함 (값으로 취급될 수 있어야 함)
3. 어떤 함수의 인자로 전달할 수 있어야 함
4. 함수의 리턴 값으로 사용될 수 있어야 함

자바스크립트의 함수는 위의 조건을 만족한다.

1. 익명 리터럴로 생성할 수 있다.
2. 변수에 할당될 수 있다.
3. 위와 같이 함수는 값이기 떄문에 인자로 전달할 수 있고 리턴값이 될 수 있다.
따라서 자바스크립트의 함수는 일급객체를 만족하는 조건을 만족

### 일급객체의 특성을 이용하여 함수 합성
```js
...
이러한 코드에서, 메인 로직에 해당하는 repeat_enhanced를 고차함수 Higher-Order Function라고 지칭한다.

printAll, printOdd, printEven와 같이 고차함수에 끼어들어가 특정 기능을 수행하는 함수를 **콜백 함수**라고 한다.