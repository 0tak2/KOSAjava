...
import BoxControl from '@/components/BoxControl.vue';
...
export default {
  ...
  methods: {
    onDeleteSelected() {
      this.selected.forEach((val, i) => {
        if (val) {
          this.kobisData = this.kobisData.filter((item) => parseInt(item.rnum, 10) - 1 !== i);
          this.selected = [];
        }
      });
    },
    ...
    onChangeDateParam(route) {
      if (route.params.date === undefined) {
        ...
        this.pickedDate = (new Date(yesterday - (new Date()).getTimezoneOffset() * 60000))
  ...
  watch: {
    $route(to, from) {
      if (to.params.date !== from.params.date) this.onChangeDateParam(to);

      if (to.params.movieCd !== from.params.movieCd) this.onChangeMovieCdParam(to);
    },
    detailMovieCd(val) {
      if (val !== '') {
